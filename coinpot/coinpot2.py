...
class Coinpot:
    ...
    def get_invest_asset(self):
        try:
            ...
            total_invest_asset = round((total_asset) * self.INVEST_ASSET_RATIO / 100) # 투자가능총자산
            
            current_invest_asset = round(total_invest_asset - total_coin_asset -locked_asset)
            if current_invest_asset < 0:
                current_invest_asset = 0

            self.asset_status['total_asset']=total_asset
            self.asset_status['total_krw_asset']=total_asset-total_coin_asset-locked_asset
            self.asset_status['total_coin_asset']=total_coin_asset
            self.asset_status['total_locked_asset']=locked_asset
            self.asset_status['total_invest_asset']=total_invest_asset
            self.asset_status['current_invest_asset']=total_invest_asset
            
            """
            _LOG_("get_invest_asset()", 
                    f"총자산:{total_asset},원화:{total_asset-total_coin_asset-locked_asset},코인:{total_coin_asset},"\
                  f"투자가능총자산:{total_invest_asset},미체결:{locked_asset},현재투자가능금액:{current_invest_asset}")
            """
    ...
    def get_holding_coin_cnt(self):
        try:
            ...
            self.holding_coin_count = len(asset_list)-1
        ...
        try:
            ...
            while True:
                ...
                if self.target_coin_list:
                    self.target_coin_list = net_change_desc(self.target_coin_list)
                
                # 검색 전략에 해당하는 coin이 있는지 확인
                self.target_coin_list = [i for i in self.target_coin_list if i['price'] < self.current_invest_asset]
            ...
            _LOG_("coin_search()", f"소요시간:{end_time-start_time}, 전량 대상 코인 수:{len(self.target_coin_list)}")
    ...
    def check_invest_possible(self):
        # 1. 보유 코인 갯수 조회
        self.get_holding_coin_cnt()
        if self.holding_coin_count < self.MAX_COIN_COUNT:
            # 2. 투자 가능 금액 조회
            self.get_invest_asset()
            print(self.asset_status)
